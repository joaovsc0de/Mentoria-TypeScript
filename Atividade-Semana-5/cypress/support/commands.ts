...
Cypress.Commands.add('verifyLoggedUser', (name: string) => {
        ...
        cy.allure().startStep('Evidência: Usuário Logado');
        cy.takeEvidence('usuário-logado');
        cy.allure().endStep();
});
...
Cypress.Commands.add('takeEvidence', (name: string) => {
  const testPath = Cypress.currentTest.titlePath.join('/');
  cy.screenshot(`${testPath}/${name}`, { capture: 'viewport' });
});
